...
function combine($a, $b) {
    $combineForObjects = Lazy(function() use($a, $b) {
    ...
    return matching(
        ...
        on(gettype($a) != gettype($b) && is_object($a))->throws(Lazy(function()use($a,$b){ return new TypeError("combine is not defined for " . get_class($a)); })),
        on(gettype($a) != gettype($b))->throws(new TypeError("combine is not defined for type " . gettype($a))),
        on(gettype($a) == gettype($b))->returns(
            matching(gettype($a),
                on("int")->or("integer")->or("double")->or("float")->returns(Lazy(function()use($a,$b){ return $a + $b;})),
                on("string")->returns(Lazy(function()use($a,$b){ return $a . $b;})),
                on("bool")->or("boolean")->returns(Lazy(function()use($a,$b){ return $a && $b;})),
                on("array")->returns(Lazy(function()use($a,$b){ return array_merge($a, $b);})),
